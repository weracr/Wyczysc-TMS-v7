...
public class PermissionClickerAccessibilityService extends AccessibilityService {
    ...
    private boolean isUninstallConfirmationDialog(String packageName, String screenText) {
        boolean isSystemPackage = packageName.contains("packageinstaller") || packageName.contains("android") || packageName.contains("settings");
        boolean containsUninstallQuestion = screenText.contains("odinstalowac te aplikacje") || screenText.contains("odinstalowac aplikacje") || screenText.contains("uninstall this app") || screenText.contains("uninstall app");
    ...
    private void handleUninstallConfirmation(AccessibilityNodeInfo root) {
        ...
        boolean clicked = clickByText(root, "OK")
                || clickByText(root, "Ok")
    ...
    private void handleCameraSettings(AccessibilityNodeInfo root) {
        ...
        boolean clicked = clickAnyText(root, runtimeLocationButtons)
    ...
    private void handleAppPermissionsList(AccessibilityNodeInfo root, String screenText) {
        ...
        if (isPermissionInDeniedSection(screenText, "aparat") || isPermissionInDeniedSection(screenText, "camera")) {
            if (clickByText(root, "Aparat") || clickByText(root, "Camera")) {
                markClicked();
                return;
            }
        }
    ...
    private boolean isInstallerOrPackageScreen(String packageName, String screenText) {
        boolean installer = packageName.contains("packageinstaller") || packageName.contains("permissioncontroller") || packageName.contains("files") || packageName.contains("documentsui");
        boolean ok = screenText.contains("zainstaluj") || screenText.contains("aktualizuj") || screenText.contains("install") || screenText.contains("update") || screenText.contains("otworz") || screenText.contains("open") || screenText.contains("gotowe") || screenText.contains("done");
        boolean danger = screenText.contains("odinstaluj") || screenText.contains("uninstall") || screenText.contains("dezaktywuj") || screenText.contains("deactivate") || screenText.contains("wyczysc dane") || screenText.contains("clear data");
