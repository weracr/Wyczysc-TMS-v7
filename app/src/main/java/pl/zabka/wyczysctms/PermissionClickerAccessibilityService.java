...
public class PermissionClickerAccessibilityService extends AccessibilityService {
    ...
    private void handleScreen(AccessibilityEvent event) {
        ...
        if (isOwnAppPackage(packageName)) {
            return;
        }
    ...
    private boolean isOwnAppPackage(String packageName) {
        if (packageName == null) {
            return false;
        }

        return packageName.equals(getPackageName().toLowerCase())
                || packageName.contains("pl.zabka.wyczysctms");
    }
    ...
    private boolean isUninstallConfirmationDialog(String packageName, String screenText) {
        ...
        boolean containsUninstallQuestion =
                screenText.contains("odinstalowac te aplikacje")
    ...
    private void handleCameraSettings(AccessibilityNodeInfo root) {
        ...
        boolean clicked =
                clickAnyText(root, runtimeLocationButtons)
    ...
    private void handleTmsAppInfoScreen(AccessibilityNodeInfo root, String screenText) {
        ...
        if (clickByText(root, "Uprawnienia")
    ...
    private void handleAppPermissionsList(AccessibilityNodeInfo root, String screenText) {
        ...
        if (isPermissionInDeniedSection(screenText, "aparat")
                || isPermissionInDeniedSection(screenText, "camera")) {

            if (tapPermissionRowByText(root, "Aparat")
                    || tapPermissionRowByText(root, "Camera")) {
        ...
        if (isPermissionInDeniedSection(screenText, "powiadomienia")
                || isPermissionInDeniedSection(screenText, "notifications")) {

            if (tapPermissionRowByText(root, "Powiadomienia")
                    || tapPermissionRowByText(root, "Notifications")) {
        ...
        if (isPermissionInDeniedSection(screenText, "lokalizacja")
                || isPermissionInDeniedSection(screenText, "location")) {

            if (tapPermissionRowByText(root, "Lokalizacja")
                    || tapPermissionRowByText(root, "Location")) {
